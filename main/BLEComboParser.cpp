...
BLEComboParser::BLEComboParser(std::string deviceName,
...
void BLEComboParser::handleMouse1(const int8_t *buf)
{
	...
	if (buf[1] & MOUSE_MIDDLE)
	{
		...
		if (should_update_scroll())
		{
			...
			m[3] = scroll_d[0] * BLECOMBOPARSER_SCROLL_MULTIPLIER / BLECOMBOPARSER_SCROLL_DIVIDER
				   / BLECOMBOPARSER_SCROLL_UPDATE_FREQ; // vertical wheel = dy
			m[4] = scroll_d[1] * BLECOMBOPARSER_SCROLL_MULTIPLIER / BLECOMBOPARSER_SCROLL_DIVIDER
				   / BLECOMBOPARSER_SCROLL_UPDATE_FREQ; // horizontal wheel = dx
	...
	else if (!(buf[1] & MOUSE_MIDDLE) && (prev_middle_stat))
	{
		...
		if (did_trigger_wheel_movement)
		{
			...
			m[1] = ((int)(buf[2])) * BLECOMBOPARSER_MOVE_MULTIPLIER
				   / BLECOMBOPARSER_MOVE_DIVIDER; // dx
			m[2] = ((int)(buf[3])) * BLECOMBOPARSER_MOVE_MULTIPLIER
				   / BLECOMBOPARSER_MOVE_DIVIDER; // dy
